<?php
    ...
    $Pass = $fName = $uEmail = $uPass =  $uPhone = $uDob = $spec = $userGender = $color = $userAllergy = $userBlood = $userspec = $uPassToDb = $days = $degree = $dd =  $dg = $userid = $fees = $userTimeErr = $errExists = $userIdErr = $userPassErr = $userFnameErr = $userEmailErr = $userPhoneErr = $userAddErr = $userAddErr = $userDobErr = $userTimeErr = $userFeesErr = $time =  $etime = $stime = $userAdd = "" ;
  ...
  <body>
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            ...
            <div>When your request status is confirmed please see the doctor.</div>
          </div>
            ...
            <div class="row row-content align-items-center">
              ...
              <div class="col-12">
                ...
                <table class="table table-hover">
                  ...
                  <?php
                    $sql=mysqli_query($conn,"select * from req_to_accountant where pid='$uid'");
                    $cnt=1;
                    while($row=mysqli_fetch_array($sql))
                    {
                      if($row['status'] == "confirmed") { $color="green";} if($row['status'] == "waiting") { $color="#cccc00";} if($row['status'] == "visited") { $color="black";} if($row['status'] == "rejected") { $color="green";}
                        // $uid = $row['userid'];
                    ?>
                    <tr>
                      ...
                      <!-- <td><button name="chs_btn" type="submit" class="btn btn-light" onClick="return confirm('Are you sure you want to proceed ?')"><i class="far fa-check-circle"></i><input type="hidden" name="id" value="<?php echo $row['userid']; ?>"/></td> </form> -->
                    </tr>
                    <?php 
                    $cnt=$cnt+1;
						}?>
